...
- **ZIP-read fallback**: On the default streaming path, `model/apksource/FallbackZipApkSource` retries with random-access `ZipFile` reading if the streaming read throws `ZipException`, for archives streaming can't handle. Enabling the ZipFile API—including through `InstallerViewModel` and `InstallerXDialogViewModel`—constructs `ZipFileApkSource` directly.
